...
# GEG6230 - Advanced Geospatial Science - Alternative Assignment 2
...
- Deadline: 26/2/21 2pm, via QMplus
...
1. Develop a workflow to calculate the factor of safety of all of the scar polygons, **using Python**, for the following conditions:
    - Using the mean scar slope
    - Using the maximum scar slope
2. Create boxplots of the `Fs` values calculated using Python for both scenarios
1. Calculate the difference between the `Fs` values in the two scenarios and plot these as a bar chart, showing one bar for each scar.
1. Write a 2 page discussion of the pros and cons of using python to automate geospatial workflows, with regard to reproducibility.

## What to submit

1. A 4 page document containing:
- **Page 1:** Boxplots of the Fs values for the 15 landslide scars (for both scenarios), **including a brief caption describing it.**
- **Page 2:** Bar chat of the difference between the Fs values in the two scenarios, **including a brief caption describing it.**
- **Pages 3-4:** Two written pages of discussion of the pros and cons of these two approaches to automation, with specific regard to reproducibility.

2. A copy of the Jupyter notebook containing the python workflow to calculate `Fs` for each catchment (under both scenarios) **AND** the code to create the graphs.
