...
Download the file `practical_8.zip` from QMplus and extract it into a sensible location (either a folder or a GeoDatabase). The zip archive contains:
...
It can often be useful to visualise the results of a clustering analysis by creating a raster surface from the dicrete point data. This process is known as interpolation, and we will cover this in detail next week.
...
- Perform 3 hot spot analyses, using 3 different distance values
- Make 3 maps, showing the differences between the hot spots identified
- Write a figure caption (approximately 250 words) describing the three maps, and the differences between the clusters identified at different distance values
